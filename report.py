...
def build_reports(a,history):
    ...
    model_rows=[[e(n),f'{bt["main"]["weights"][n]*100:.2f}%',f'{bt["main"]["weight_diagnostics"][n]["hit20"]:.3f}',f'{bt["main"]["weight_diagnostics"][n]["hit60"]:.3f}',f'{bt["main"]["weight_diagnostics"][n]["hit120"]:.3f}',bt["main"]["weight_diagnostics"][n]["zero_hit_streak"],f'{bt["main"]["model_logloss"][n]:.6f}'] for n in bt["main"]["names"]]
...
<section id="models" class="tab"><h2>520期時間序列走步回測</h2><div class="cards"><article><b>主號12碼平均命中</b><strong>{bt['main']['avg_hits']}</strong><small>隨機基準 1.4694</small></article><article><b>特別號3碼命中</b><strong>{bt['special']['avg_hits']}</strong><small>隨機基準 0.0612</small></article><article><b>最大模型權重</b><strong>{a['release_gate']['max_main_weight']*100:.1f}%</strong></article></div><h2>全新20／60／120期多尺度權重</h2><p>{e(a.get('weight_logic',{}).get('reason',''))}</p>{table(['模型','最終權重','近20命中','近60命中','近120命中','連續零命中','對數損失'],model_rows)}<h2>上期主號逐模型錯誤追責</h2>{table(['模型','原第一名','1中1','前12命中','命中號','權重變動','處置'],module_review_rows)}<h2>上期特別號逐模型錯誤追責</h2>{table(['模型','原第一名','1中1','前三命中','權重變動','處置'],special_review_rows)}<h2>失敗回饋規則</h2><ul><li>最近20期權重50%、60期30%、120期20%</li><li>每個錯誤模型必須逐一追責並重新校準</li><li>連續零命中與不穩定模型立即懲罰降權</li><li>單一模型權重上限25%</li><li>主號與特別號完全分開校準</li><li>最強獨支每期只能有一個，且必須在開獎前封存</li></ul></section>
