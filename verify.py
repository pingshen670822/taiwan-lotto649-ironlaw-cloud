...
def main():
    ...
    add("top9_boundary_adaptive_weights",logic.get("windows")==[20,60,120] and logic.get("target_cutoff")==9 and logic.get("spill_range")==[10,15] and logic.get("adaptive_rank_fusion") is True and logic.get("adaptive_boundary_rotation") is True and logic.get("boundary_rotation_cooldown_draws")==2 and logic.get("candidate_failure_tenure") is True and logic.get("latest_draw_weight_recalculation") is True and logic.get("failure_streak_penalty") is True and logic.get("stability_penalty") is True and logic.get("single_model_cap")==.25,json.dumps(logic,ensure_ascii=False))
    ...
    add("no_fake_or_future_data",integrity.get("official_rows")==len(draws) and integrity.get("future_data_used") is False and integrity.get("previous_prediction_rewritten") is False and integrity.get("prediction_revision_append_only") is True,json.dumps(integrity,ensure_ascii=False))
